...
- `$maxD2` — interaction cutoff in squared lattice units. **Must be strictly greater than `physLen²`** (the LJ zero-crossing) to include any attractive interactions. The LJ minimum is at `d²=2^(1/3)·physLen²≈1.26·physLen²`. `Ceiling[2*physLen^2]` places the cutoff just past the minimum (`d=√2·physLen≈1.41·physLen`); for the full 2.5σ attractive tail use `Ceiling[6.25*physLen^2]`. Setting `$maxD2 ≤ physLen²` excludes all attraction. Default `$maxD2=Infinity` is correct for the symbolic checker (small lattices have finitely many pairs anyway); override for large-lattice animation runs.
